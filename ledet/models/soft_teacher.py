...
from ledet.utils import log_image_with_boxes, log_every_n
from ledet.utils.structure_utils import (
    dict_split,
    weighted_loss,
    sigmoid_schedule,
)
...
class SoftTeacher(MultiStreamDetector):
    def __init__(self, model: dict, train_cfg=None, test_cfg=None):
        ...
        if train_cfg is not None:
            self.freeze("teacher")
            self.unsup_weight_alpha = self.train_cfg.unsup_weight_alpha
            self.unsup_weight_warmup = self.train_cfg.unsup_weight_warmup
            self.debug_mode = self.train_cfg.get("debug_mode", False)
            self.num_classes = self.student.roi_head.bbox_head.num_classes

    def forward_train(self, img, img_metas, **kwargs):
        super().forward_train(img, img_metas, **kwargs)
        return self.forward_unsup_train(img, img_metas, **kwargs)
    
    def forward_unsup_train(self, img, img_metas, **kwargs):
        ...
        if "unsup_student" in data_groups:
            # We obtain the counter for `self.curr_step` from the MeanTeacher Hook.
            self.loss_weight = sigmoid_schedule(self.curr_step, warmup=self.unsup_weight_warmup)
            unsup_loss_weight = self.loss_weight * self.unsup_weight_alpha
            self.student_info, self.teacher_info = self.get_data_info(
                data_groups["unsup_student"], data_groups["unsup_teacher"]
            )
            unsup_loss = weighted_loss(
                self.compute_pseudo_label_loss(self.student_info, self.teacher_info),
                unsup_loss_weight,
            )
    ...
    def get_data_info(self, student_data, teacher_data):
        ...
        # Track the transformation matrix M mapping from teacher src to student dst
        student_info["student_matrix"] = self._get_trans_mat(
            teacher_info["transform_matrix"], student_info["transform_matrix"]
        )
        # Track the transformation matrix M mapping from student src to teacher dst
        teacher_info["teacher_matrix"] = self._get_trans_mat(
    ...
    def unsup_rcnn_cls_loss(
        ...
        **kwargs,
        ...
        student_info["candidate_proposals"] = selected_bboxes
        # Transform student bboxes to teacher space using teacher matrix
        M_teacher = teacher_info["teacher_matrix"]
        aligned_proposals = self._transform_bbox(
            selected_bboxes,
            M_teacher,
            [meta["img_shape"] for meta in teacher_info["img_metas"]],
        )
        teacher_info["candidate_proposals"] = aligned_proposals
        with torch.no_grad():
            ...
            neg_inds = assigned_label == self.num_classes
        ...
        loss["loss_bbox"] = loss["loss_bbox"].sum() / max(bbox_targets[1].size()[0], 1.0)
    ...
    def unsup_rcnn_reg_loss(
        ...
        **kwargs,
        ...
        if self.debug_mode and student_info is not None:
            if len(gt_bboxes[0]) > 0:
                log_image_with_boxes(
                    interval=500,
                    tag="rcnn_reg",
